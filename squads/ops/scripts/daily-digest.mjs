 ...
 *
 * Runs end-of-day to:
 * 1. Summarize day's activities (Ollama local = €0)
 * 2. Extract decisions → .aios/memory/decisions/
 * 3. Extract patterns → .aios/learning/patterns/
 * 4. Update metrics
 *
 * Usage:
...
function generateDeterministicSummary(entries) {
  ...
  const summary = [];
  if (byType.decision) summary.push(`${byType.decision} decisões tomadas`);
  if (byType.implementation) summary.push(`${byType.implementation} implementações`);
  if (byType.error) summary.push(`${byType.error} erros/fixes`);
  if (byType.action) summary.push(`${byType.action} ações gerais`);
...
function generateCorrectiveActions(entries, errors, patterns) {
  const actions = [];

  // Rule 1: High error count
  if (errors.length >= 3) {
    actions.push('🔴 **High error count** — 3+ errors today. Review error patterns and add pre-flight checks.');
  }

  // Rule 2: Repeated patterns suggest automation opportunity
  const highRepeat = patterns.filter(p => p.count >= 3);
  if (highRepeat.length > 0) {
    actions.push(`🔄 **Repetitive work detected** — "${highRepeat[0].action}" done ${highRepeat[0].count}x. Consider automating or templating.`);
  }

  // Rule 3: Too many decisions in one day (decision fatigue)
  const decisionCount = entries.filter(e => e.type === 'decision' || e.tags?.includes('auto-detected-decision')).length;
  if (decisionCount >= 5) {
    actions.push('🧠 **Decision fatigue risk** — 5+ decisions today. Document patterns to reduce future decision load.');
  }

  // Rule 4: No activity logged (if called with empty log, this won't trigger, but useful for cross-day)
  if (entries.length === 0) {
    actions.push('⚠️ **No activity logged** — Verify auto-capture hooks are working.');
  }

  // Rule 5: Many files touched without decisions logged
  const files = [...new Set(entries.flatMap(e => e.files || []))];
  if (files.length > 10 && decisionCount === 0) {
    actions.push('📝 **Many changes, no decisions logged** — Large changes should have documented rationale.');
  }

  // Default positive
  if (actions.length === 0) {
    actions.push('✅ **No corrective actions needed** — Process running within expected parameters.');
  }

  return actions;
}

function generateDigest(date, entries, summary, decisions, errors, patterns, actions = []) {
...
---
*Generated by daily-digest.mjs | Corrective actions: deterministic (€0)*
`;
}
...
async function run(options = {}) {
  ...
  // Corrective action suggestions (closed-loop — Board recommendation 2026-02-15)
  const actions = generateCorrectiveActions(entries, errors, patterns);

  // Generate digest
  const digest = generateDigest(date, entries, summary, decisions, errors, patterns, actions);
...
What it does:
  ...
  5. Generates summary (Ollama local = €0, or deterministic fallback)
  6. Saves digest to .aios/logs/digests/
  7. Updates memory indexes and learning metrics

Cost: €0 (uses Ollama local or deterministic processing)
`);
  process.exit(0);
}
